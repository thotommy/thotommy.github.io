...
export class ParallaxDirective {
  ...
  onWindowScroll(event) {
    ...
    if (window.scrollY < 20) {
      this.startTransition.emit({startTransition: false, element: this.eleRef.nativeElement} as ParallaxTransition);
      return;
    }
    if (this.eleRef.nativeElement.getBoundingClientRect().top < -41) {
      this.startTransition.emit({startTransition: true, element: this.eleRef.nativeElement} as ParallaxTransition);
      return;
    }
